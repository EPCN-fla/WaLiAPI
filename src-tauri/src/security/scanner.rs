...
fn walk_json(value: &serde_json::Value, phase: &str, path: &str, ctx: &mut ScanContext) -> Result<(), BudgetError> {
    if ctx.findings.len() >= MAX_FINDINGS { return Ok(()); }
    ctx.check_budget()?;
    ...
    match value {
        ...
        serde_json::Value::Array(items) => {
            ...
            for (i, item) in items.iter().enumerate() {
                ...
                if ctx.findings.len() >= MAX_FINDINGS { break; }
            }
...
fn scan_text(text: &str, phase: &str, location: &str, ctx: &mut ScanContext) {
    ...
    if ctx.settings.scan_unicode { scan_unicode(scan_text, phase, location, ctx); }
    if ctx.settings.scan_network { scan_network(scan_text, phase, location, ctx); }
    if ctx.settings.scan_tools { scan_tool_risks(scan_text, phase, location, ctx); }
...
fn scan_credentials(text: &str, phase: &str, location: &str, ctx: &mut ScanContext) {
    for token in split_candidates(text) {
        ...
        if is_secret {
            add(&mut ctx.findings, phase, "credential", "credential.secret_token", RiskLevel::High, "发现疑似密钥/Token", "请求内容中出现 API Key、Bearer Token、GitHub Token、JWT 或云厂商密钥样式字符串。", location, t);
            break;
        }
    }

    if text.contains("-----BEGIN OPENSSH PRIVATE KEY-----") || text.contains("-----BEGIN RSA PRIVATE KEY-----") || text.contains("-----BEGIN PRIVATE KEY-----") {
        add(&mut ctx.findings, phase, "credential", "credential.private_key", RiskLevel::Critical, "发现私钥内容", "请求内容中包含私钥 PEM/OpenSSH 头部，存在严重凭证泄露风险。", location, "-----BEGIN PRIVATE KEY-----");
    }

    let lower = text.to_ascii_lowercase();
    for key in ["authorization:", "cookie:", "sessionid=", "auth_token=", "secret_key", "access_key", "database_url"] {
        if lower.contains(key) {
            add(&mut ctx.findings, phase, "credential", "credential.named_secret", RiskLevel::High, "发现敏感凭证字段", "请求内容包含 Authorization、Cookie、Session 或 Secret 字段名。", location, key);
            break;
        }
    }
}
...
fn scan_paths(text: &str, phase: &str, location: &str, ctx: &mut ScanContext) {
    ...
    for item in sensitive {
        if lower.contains(item) {
            add(&mut ctx.findings, phase, "file", "file.sensitive_path", RiskLevel::High, "发现敏感文件路径", "内容引用了 .env、SSH 私钥、云凭证或包管理器凭证等敏感路径。", location, item);
            break;
        }
    }
    if text.contains("/Users/") || text.contains("C:\\Users\\") || text.contains("/home/") {
        add(&mut ctx.findings, phase, "infra", "infra.local_path", RiskLevel::Medium, "发现本地用户路径", "内容包含本地用户目录路径，可能暴露用户名、项目结构或机器信息。", location, snippet(text));
    }
}
...
fn scan_unicode(text: &str, phase: &str, location: &str, ctx: &mut ScanContext) {
    ...
    if zero_width > 0 {
        add(&mut ctx.findings, phase, "unicode", "unicode.zero_width", RiskLevel::Medium, "发现零宽 Unicode 字符", "内容包含不可见零宽字符，可能用于隐藏标记或混淆文本。", location, &format!("zero_width_count={}", zero_width));
    }
    if bidi > 0 {
        add(&mut ctx.findings, phase, "unicode", "unicode.bidi_control", RiskLevel::High, "发现方向控制 Unicode 字符", "内容包含 Bidi 方向控制字符，可能改变代码、URL 或命令的视觉顺序。", location, &format!("bidi_count={}", bidi));
    }
    if variation > 0 {
        add(&mut ctx.findings, phase, "unicode", "unicode.variation_selector", RiskLevel::Medium, "发现变体选择符", "内容包含 Unicode variation selector，可能被用于隐写编码。", location, &format!("variation_count={}", variation));
    }
}
...
fn scan_network(text: &str, phase: &str, location: &str, ctx: &mut ScanContext) {
    ...
    for domain in ip_probe {
        if lower.contains(domain) {
            add(&mut ctx.findings, phase, "network", "network.ip_probe", RiskLevel::High, "发现公网 IP 探测服务", "内容引用公网 IP 查询服务，可能用于识别真实出口 IP 或代理状态。", location, domain);
            break;
        }
    }
    ...
    for domain in suspicious {
        if lower.contains(domain) {
            add(&mut ctx.findings, phase, "network", "network.suspicious_domain", RiskLevel::High, "发现可疑外联域名", "内容引用临时 Webhook、隧道或文件投递服务，可能用于接收外传数据。", location, domain);
            break;
        }
    }
    if lower.contains("http://") || lower.contains("https://") {
        add(&mut ctx.findings, phase, "network", "network.external_url", RiskLevel::Info, "发现外部 URL", "内容包含外部 URL，建议结合上下文判断是否为正常请求或外联风险。", location, first_url(text).unwrap_or_else(|| "http(s) URL".to_string()).as_str());
    }
}
...
fn scan_tool_risks(text: &str, phase: &str, location: &str, ctx: &mut ScanContext) {
    ...
    if reads_sensitive && network {
        add(&mut ctx.findings, phase, "tool", "tool.shell.exfiltration", RiskLevel::Critical, "疑似敏感数据外传命令", "内容同时包含敏感文件/环境路径读取与外部网络传输特征，存在严重外泄风险。", location, snippet(text));
    }
}
...
fn snippet(text: &str) -> &str {
    let max = 160;
    if text.len() <= max { text } else { truncate_utf8(text, max) }
}
...
mod scanner_tests {
    ...
    fn many_small_strings_cannot_bypass_string_node_budget() {
        let settings = SecuritySettings::default();
        let budget = ScanBudget { max_string_nodes: Some(8), ..Default::default() };
        let arr: Vec<serde_json::Value> = (0..20).map(|i| serde_json::json!({"x": format!("val{}", i)})).collect();
    ...
    fn depth_budget_is_enforced() {
        let settings = SecuritySettings::default();
        let budget = ScanBudget { max_depth: Some(4), ..Default::default() };
    ...
    fn oversized_string_respects_per_string_cap_without_panic() {
        let settings = SecuritySettings::default();
        let budget = ScanBudget { max_text_bytes_per_string: Some(16), ..Default::default() };
    ...
    fn time_budget_fails_closed_on_tiny_budget() {
        let settings = SecuritySettings::default();
        let budget = ScanBudget { max_elapsed: Some(Duration::from_nanos(1)), ..Default::default() };
